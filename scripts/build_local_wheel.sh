...
# py3-none-any fallback + a separately-installed supercoder_index_native).
#
# Usage:  bash scripts/build_local_wheel.sh
# Output: dist-merged/cluxion_agentplugin_supercoder-<ver>-<platform>.whl
set -euo pipefail
...
( cd rust/supercoder_index && maturin build --release --strip --out dist >/dev/null )

PURE="$(ls -t dist/cluxion_agentplugin_supercoder-*-py3-none-any.whl | head -1)"
NATIVE="$(ls -t rust/supercoder_index/dist/supercoder_index_native-*.whl | head -1)"
echo "      pure   = $PURE"
echo "      native = $NATIVE"
